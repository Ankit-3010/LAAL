...
# ---------------------------------------------------------------------------
# Neo4j Graph DB logic (REMOVED in LAAL standalone)
# ---------------------------------------------------------------------------
...
def _graph_update_bg(*args, **kwargs):
    pass
...
def parse_target(target: str, subdomain_list: list = None) -> dict:
    ...
    """
    # Sanitize target: strip protocol, www., trailing slashes, port
    # Allows users to pass "https://www.vulnhub.com/" and still get "vulnhub.com"
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="LAAL - RedAmon Standalone Recon Engine")
    parser.add_argument("--domain", help="Target root domain (e.g. example.com)")
    parser.add_argument("--subdomains", help="Comma-separated list of subdomain prefixes")
    parser.add_argument("--ip", help="Comma-separated list of target IPs/CIDRs")
    args = parser.parse_args()

    # Override settings with CLI arguments if provided
    global _settings, TARGET_DOMAIN, SUBDOMAIN_LIST, IP_MODE, TARGET_IPS
    if args.domain:
        _settings['TARGET_DOMAIN'] = args.domain
        TARGET_DOMAIN = args.domain
    if args.subdomains:
        subs = [s.strip() for s in args.subdomains.split(',')]
        _settings['SUBDOMAIN_LIST'] = subs
        SUBDOMAIN_LIST = subs
    if args.ip:
        ips = [i.strip() for i in args.ip.split(',')]
        _settings['TARGET_IPS'] = ips
        _settings['IP_MODE'] = True
        TARGET_IPS = ips
        IP_MODE = True

    if not TARGET_DOMAIN and not (IP_MODE and TARGET_IPS):
        print("[!] Error: No target specified. Use --domain <domain> or --ip <ips>")
        return 1

    start_time = datetime.now()
    # ... rest of the existing main() code ...
    ...
    # Propagate sanitized domain back so all modules use the clean value
    if root_domain != TARGET_DOMAIN:
        print(f"[*][Pipeline] Sanitized target: {TARGET_DOMAIN} → {root_domain}")
        TARGET_DOMAIN = root_domain
        _settings['TARGET_DOMAIN'] = root_domain
