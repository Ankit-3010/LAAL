...
# If first argument starts with '-' (e.g. --domain), assume we want to run the main pipeline
if [[ "$1" == -* ]]; then
    echo -e "${YELLOW}[*] Arguments detected, running main pipeline with: python /app/recon/main.py $@${NC}"
    exec python /app/recon/main.py "$@"
else
    # Otherwise execute the command as provided (e.g. /bin/bash or python ...)
    exec "$@"
fi
